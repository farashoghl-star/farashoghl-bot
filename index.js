...
function normalizePhone(input) {
  ...
  if (/^0\d{10}$/.test(s)) return s.replace(/^0/, "+98");
...
function askNext(ctx) {
  ...
  return ctx.reply(
    ...
    Markup.inlineKeyboard([
      [
        Markup.button.callback("✅ بله", "ans_yes"),
        Markup.button.callback("❌ خیر", "ans_no")
      ]
    ])
  );
}
...
bot.on("contact", async (ctx) => {
  ...
  const type = getTypeByScore(st.score) || TYPES[3];
  await sendGiftAndLead(ctx, type, phone);
});

// ---- Handle text phone (desktop)
bot.on("text", async (ctx) => {
  const st = userState.get(ctx.from.id);
  if (!st || !st.awaitingPhone) return;
...
async function sendGiftAndLead(ctx, type, phone) {
  ...
  await ctx.reply(`✅ شماره‌ات ثبت شد: ${phone}`);
  ...
  const lead = [
    ...
    `یوزرنیم: @${u.username || "—"}`,
    `ID: ${u.id}`,
    `امتیاز: ${userState.get(u.id).score}/${QUESTIONS.length}`,
    `تیپ: ${type.title}`,
    `موبایل: ${phone}`,
    `پاسخ‌ها: ${userState.get(u.id).answers.join(", ")}`
  ].join("\n");
